...
impl Grid {
    fn new(w: usize, h: usize) -> Self {
        Grid {
            width: w,
            height: h,
            points: vec![0; (w * h) as usize],
        }
    }

    fn process_cmd(&mut self, cmd: &Command) {
        let mut exec = |b: &Block, cb: &dyn Fn(usize) -> usize| {
        ...
        match cmd {
            Command::Toggle(b) => exec(b, &|val: usize| val + 2),
            Command::On(b) => exec(b, &|val: usize| val + 1),
            Command::Off(b) => exec(b, &|val: usize| {
                if val > 1 {
                    val - 1
                } else {
                    0
